...
<script>
	export default{
		...
		methods:{
    		...
    		findOne(id){
    			this.$http.post('typesuffix/findOne',{
    				"id":id,
					...
					if(data.code==0){
						...
						this.form.suffix=data.data.suffix;
						
						this.findSelectedComponent(data.data.suffix);
    		...
    		findSelectedComponent(suffix){
				this.loading=true;
				this.$http.post('typesuffix/findOperateBySuffix',{
					"suffix":suffix,
					"token":sessionStorage.getItem("token")
				}).then(response => {
					var data=response.body;
					if(data.code==0){
						...
						for(var i=0;i<data.data.length;i++){
							arrs.push(data.data[i].componentcode);
						}
						this.form.json=arrs;
					}else{
						this.alertMsg(data.code,data.msg);
        			...
        			if (valid) {
        				this.$confirm('此操作将修改表单, 是否继续?', '提示', {
		        			...
		        			formData.append('id', this.form.id);
      						...
      						this.loading=true;
	        				this.$http.post('typesuffix/update',formData,{
								headers: { 'Content-Type': 'multipart/form-data' }
							}).then(response => {
								var data=response.body;
								if(data.code==0){
							        this.reset();
							        this.$emit('handleResult',data.code);
								}
								this.alertMsg(data.code,data.msg);
								this.loading=false;
							});
